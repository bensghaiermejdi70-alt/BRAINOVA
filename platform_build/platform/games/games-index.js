const GamesIndex = (()=>{
  ...
  const DEFAULT_PAYMENT_LINK = './brainova.html?bundle_purchased=1';
  Object.keys(meta).forEach(k=>{
  ...
  function launch(id){
    const m = meta[id] || {};
  // Resolve the file URL relative to this module so games open correctly
  // whether the platform is served from a numeric IP, a subpath, or file://
  // If a global SITE_BASE is defined by the platform shell, prefer that
  const siteBase = (typeof window !== 'undefined' && window.SITE_BASE) ? window.SITE_BASE : '/';
  let raw = m.file ? m.file : `../jeux${id}.html`;
    let url = raw;
    try{
      // Try resolving the raw path using multiple conservative bases so the
      // platform works when served from an IP, a subpath, or via the simple
      // static server. Order: page-relative -> '/platform/games/' -> filename fallback.
      try{
        // 1) page-relative
        url = new URL(raw, window.location.href).href;
      }catch(e){ url = raw; }
      try{
        // 2) site-base platform/games (helps when platform is hosted under a subpath)
        const candidate = new URL(siteBase + 'platform/games/' + raw.replace(/^\.\/?/, ''), window.location.origin).href;
        if(candidate) url = candidate;
      }catch(e){ /* ignore */ }
      // 3) Normalize to './filename' so the simple_static_server can serve it directly
      try{
        const u = new URL(url, window.location.href);
        const filename = u.pathname.split('/').pop();
        if(filename) url = './' + decodeURIComponent(filename);
      }catch(e){ /* ignore */ }
    }catch(e){
  ...
  try{ if(typeof window !== 'undefined'){ const g = window.GamesIndex || {}; g._lastResolved = url; window.GamesIndex = g; } }catch(e){}

    // If the platform was opened from the filesystem (file://) many browsers block
    // iframe embedding or relative paths may resolve incorrectly. In that case we
    // already normalized above to a './filename' when possible.
...
try{ if(typeof window !== 'undefined' && !window.GamesIndex){ window.GamesIndex = GamesIndex; } }catch(e){}
