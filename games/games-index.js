const GamesIndex = (()=>{
  const meta = {
  0: { id: 0, title: 'Test QI', description: 'Test QI — Carte de vérification (Cartridge: carte_qi.html)', icon: '🧪', tags: ['test','qi'], players: '1', duration: '1-5 min', file: './carte qi.html' },
  ...
  async function launch(id){
    ...
    // Candidate locations to try (prefer the declared raw, then root, then common folders)
    const basename = (raw + '').split('/').pop();
    const candidates = [raw, `./${basename}`, `./platform_canonical/${basename}`, `./docs/${basename}`];

    // Helper to normalize candidate to an absolute URL
    const toUrl = cand => {
      try{ return new URL(cand, window.location.href).href; }catch(e){ return cand; }
    };

    // Try each candidate with a HEAD request (same-origin). Use GET fallback if HEAD not allowed.
    let foundUrl = null;
    for(const cand of candidates){
      const url = toUrl(cand);
      try{
        // Attempt a lightweight fetch to check existence
        const resp = await fetch(url, { method: 'HEAD', cache: 'no-store' });
        if(resp && resp.ok){ foundUrl = url; break; }
      }catch(e){
        try{
          const resp2 = await fetch(url, { method: 'GET', cache: 'no-store' });
          if(resp2 && resp2.ok){ foundUrl = url; break; }
        }catch(_){ /* ignore and try next */ }
      }
    }

    // If none found, fall back to resolved raw URL
    if(!foundUrl) foundUrl = toUrl(raw);
    if(window && window.console){ console.debug(`GamesIndex.launch id=${id} chosen='${foundUrl}'`); }

    if(window && window.OverlayManager) return window.OverlayManager.open(foundUrl, title);
